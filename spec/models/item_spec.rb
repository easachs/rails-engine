...
RSpec.describe Item, type: :model do
  ...
  describe 'class methods' do
    ...
    it 'finds items array by max and min price' do
      ...
      expect(Item.price_range(4.75, 7.50).count).to eq(3)
      expect(Item.price_range(4.75, 7.50).first).to eq(cabbage)
      expect(Item.price_range(4.75, 7.50).last).to eq(sprinkles)
      expect(Item.price_range(10.01, 11.0)).to eq([])
      expect(Item.price_range(0.01, 1.99)).to eq([])
      expect(Item.price_range(5.0, 4.0)).to eq([])
    end
    ...
    it 'finds items array by min price' do
      ...
      expect(Item.min_price(7.25).count).to eq(2)
      expect(Item.min_price(7.25).first).to eq(candy)
      expect(Item.min_price(7.25).last).to eq(casserole)
      expect(Item.min_price(10.01)).to eq([])
      expect(Item.min_price(0.01).count).to eq(6)
    end
    ...
    it 'finds items array by max price' do
      ...
      expect(Item.max_price(3.0).count).to eq(2)
      expect(Item.max_price(3.0).first).to eq(rainbow)
      expect(Item.max_price(3.0).last).to eq(unicorn)
      expect(Item.max_price(0.01)).to eq([])
      expect(Item.max_price(20.0).count).to eq(6)
    end
  end
end
